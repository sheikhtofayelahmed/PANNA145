...
export default function AgentPage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      const myRecords = (gameJson.data || []).filter(
        (r) => r.agentId === agentId,
      );
      ...
      const myLost = (lostJson.lostData || []).find(
        (l) => l.agentId === agentId,
      );
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-5">
          ...
          <button
            ...
            className="text-xs text-gray-500 hover:text-gray-700 border border-gray-300 px-3 py-1 rounded-lg transition">
            Logout
          </button>
        </div>
          ...
          <>
            ...
            {gameData.length > 0 &&
              (() => {
                ...
                const rawGame = gameData.reduce(
                  (s, r) => s + (r.totalGame || 0),
                  0,
                );
                const rawWin = gameData.reduce(
                  (s, r) =>
                    s +
                    (r.totalWin?.panna || 0) * 145 +
                    (r.totalWin?.single || 0) * 9 +
                    (r.totalWin?.jodi || 0) * 80,
                  0,
                );
                ...
                // Debt adjustment: same side (BANKER+banker owes, AGENT+agent owes) → add; opposite → subtract
                const debtAmount = debt?.amount || 0;
                const sameSide =
                  debt &&
                  ((tag === "BANKER" && debt.type === "agent_gets") ||
                    (tag === "AGENT" && debt.type === "banker_gets"));
                const debtAdj = debt
                  ? sameSide
                    ? debtAmount
                    : -debtAmount
                    ...
                    <table
                      ...
                      <tbody>
                        <tr className="hover:bg-gray-50">
                          <td
                            className={`${std} text-center text-gray-400 text-xs`}>
                            1
                          </td>
                          ...
                          <td className={`${std} text-right`}>
                            <div
                              className={`font-mono font-bold ${tag === "BANKER" ? "text-green-700" : "text-red-600"}`}>
                              {fmt(Math.abs(pl))}
                            </div>
                            {applyWinDisc && winDiscAmount > 0 && (
                          ...
                          <td className={`${std} text-center`}>
                            <span
                              className={`text-xs font-bold px-2 py-0.5 rounded ${tag === "BANKER" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"}`}>
                              {tag}
                            </span>
                          </td>
                        </tr>
                        ...
                        {debt && debtAmount > 0 && (
                          <tr className="bg-yellow-50">
                            <td className={`${std}`}></td>
                            <td
                              className={`${std} text-xs text-yellow-700 font-semibold`}>
                              Debt
                              {debt.note ? (
                            ...
                            <td className={`${std}`}></td>
                            <td
                              className={`${std} text-right font-mono text-xs`}>
                              <span
                                className={
                                  sameSide ? "text-green-700" : "text-red-600"
                                }>
                                {sameSide ? "+" : "−"}
                                {fmt(debtAmount)}
                              </span>
                              <div className="text-gray-400 text-xs">
                                {debt.type === "banker_gets"
                                  ? "Agent owes"
                                  : "Banker owes"}
                              </div>
                            </td>
                      ...
                      <tfoot>
                        <tr className="border-t-2 border-gray-400 bg-gray-50 font-bold">
                          ...
                          <td className={`${std} text-right font-mono`}>
                            {fmt(effectiveWin + winDiscAmount)}
                          </td>
                          <td
                            className={`${std} text-right font-mono font-bold ${finalTag === "BANKER" ? "text-green-700" : "text-red-600"}`}>
                            {fmt(Math.abs(finalPL))}
                          </td>
                          <td className={`${std} text-center`}>
                            <span
                              className={`text-xs font-bold px-2 py-0.5 rounded ${finalTag === "BANKER" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"}`}>
                              {finalTag}
                            </span>
                          </td>
                        </tr>
            ...
            <div className="border border-gray-200 rounded-xl p-4 mb-3">
              ...
              {!debt ? (
                ...
                <div>
                  <div
                    className={`text-2xl font-mono font-bold ${debt.type === "banker_gets" ? "text-red-600" : "text-green-700"}`}>
                    {fmt(debt.amount)}
                  </div>
                  <div
                    className={`text-xs mt-0.5 font-semibold ${debt.type === "banker_gets" ? "text-red-500" : "text-green-600"}`}>
                    {debt.type === "banker_gets"
                      ? "You owe Banker"
                      : "Banker owes You"}
                  </div>
